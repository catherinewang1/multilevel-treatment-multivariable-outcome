...
#' Plot some approximating matrices when approx_matrix is called
plot_approx_matrix <- function(matapprox, save_individual_rank_plots, color_limits) {

} 



#' Create an ordering for the rows and columns for all the future plots 
#' Currently order based on marginal hclust-ering 
#' @param mat (matrix) numeric matrix to form a row and column ordering for
#'  rows are assumed to be grna and cols are assume to be gene
#'  rows and cols should be named to decrease mistakes later on 
#' @param save_folder (character) path to the save folder (location to save the resulting ordering)
#'  which will be named <save_folder>/grna_gene_ordering.rds
#' @param include_mat_tall (boolean) whether to include a dataframe with mat_tall (usually not)
#' @return list with: grna_idx and gene_idx, each has the name of the original grna or gene, 
#'         and the ordered idx of that grna or gene
#'    and mat_tall tall dataframe version of mat (should be the original) if requested (include_mat_tall)
plot_set_rowcol_order <- function(mat, save_folder=NULL, include_mat_tall=FALSE) {
  # - get a row and col order using clustering (marginall), for plotting later
  # - tall version of mat matrix
  

  # order rows and cols based on hclust (this method could be changed in the future. This is only for plotting purposes)
  ...
  grna_gene_ordering = list(grna_index=grna_index, 
                            gene_index=gene_index)
  if(include_mat_tall) {
    # tall version of mat matrix (with row and col idx)
    ...
    # add to result
    grna_gene_ordering$mat_tall = mat_tall
  }


  if(!is.null(save_folder) && dir.exists(save_folder)) {
    saveRDS(object = grna_gene_ordering, 
            file = sprintf('%s/grna_gene_ordering.rds', save_folder))
  }
  
  return(grna_gene_ordering)
}
...
#' or ...
#' 
#' now very similar to h3_1_approximate_matrix
...
#' @param save_folder (string) or NULL for no plots
#' @param save_individual_rank_plots (boolean) if save_folder is specified, 
...
#' @param grna_gene_ordering (list) of orders and
#' @example
#' lowrankmatrices = approx_lowrank(
#'        mat = matrices$estimates,
#'        ranks = c(1, 3, 5, 10, 15),
#'        save_folder = '../plots/matrix/lowrank/estimate/',
#'        color_limits = c(-2, 2)
#' )
approx_matrix <- function(mat, method, ranks, save_folder=NULL, save_individual_rank_plots=FALSE, color_limits = c(-2, 2), methodParams=NULL, 
  ...
  # method = 'sparseSVD'
  # ranks = c(1, 3, 5, 10, 20)
  # save_folder = "../plots/matrix/sparseSVD/"
  # color_limits = c(-2, 2)
  ...
  # use given grna and gene ordering, or else do by given matrix form... (will be mixed up and hard to see)
  ...
  if(method == 'SVD' || method == 'sparseSVD' || method == 'sparseSVD_autoparams') {
    ...
    # maybe softImpute also can also do this? I think not though
    ...
    if(method == 'SVD') {
        ...
        rm(svd_res); gc(verbose=FALSE)
    } else if (method == 'sparseSVD') {
        # Sparse SVD: sparse low rank approximation --------------------------------------------------------------------
        ...
        rm(cv.out, pmd_res); gc(verbose=FALSE)
    }
    
    # plot the singular values for SVD and spSVD methods
    if(!is.null(save_folder) && dir.exists(save_folder)) { 
      # svd_res$d |> plot()
      ggplot(NULL, aes(x = 1:length(d), # plot the singular values
                       y = d)) +
        geom_point() +
      ...
      ggsave(sprintf('%s/singvals.pdf', save_folder), height = 4, width = 6)
    }
    ...
    #       ggsave(sprintf('%s/rank=%02d.pdf', save_folder, r), height = 6, width = 3)
    #     }
    ...
    #   ggsave(sprintf('%s/rankall.pdf', save_folder), height = 6, width = 1.5*(length(ranks) + 1))
    ...
    # }
  } else if(method == 'sparseSVDauto') {
    # TODO:
    # --- Sparse SVD finding params automatically using cross validation
    approxmatrices = list()
    cv.out  = PMA::SPC.cv(mat, sumabsvs = seq(1.2, min(5, sqrt(nrow(mat)), sqrt(ncol(mat))), len = 10), trace=FALSE)
    pmd_res = PMA::SPC(mat, sumabsv=cv.out$bestsumabsv, K = max(ranks), trace=FALSE)
    for(r in ranks) {
      ...
      row.names(cur_matapprox) = row.names(mat); colnames(cur_matapprox) = colnames(mat)
      approxmatrices[[r]] = cur_matapprox
      rm(cur_matapprox)
    }
    ...
    for(r in ranks) {
      ...
      approxmatrices[[r]] = biclust_mat
    }    

  } else if (method == 'zeros') {
    # TODO:
    # --- zero 0
    approxmatrices = matrix(0,                     nrow = nrow(mat), ncol = ncol(mat), dimnames = list(row.names(mat), colnames(mat)))
  } else if(method == 'average') {
    # TODO:
    # --- overall average 
    approxmatrices = matrix(mean(mat, na.rm=TRUE), nrow = nrow(mat), ncol = ncol(mat), dimnames = list(row.names(mat), colnames(mat)))
  } else {
  ...
  if(!is.null(save_folder) && dir.exists(save_folder)) {
    ...
    # load in premade gene_idx and grna_idx from input grna_gene_ordering
    if(!is.null(grna_gene_ordering)) {
      gene_index = grna_gene_ordering$gene_index
      grna_index = grna_gene_ordering$grna_index
    } else { # else use the original mat
    ...
    # make tall dataframe for original values from mat
    mat_tall = mat |> as.data.frame() |> tibble::rownames_to_column('grna') |> 
      tidyr::pivot_longer(cols = 2:(ncol(mat)+1), 
                          names_to = 'gene', values_to = 'value') |>
      mutate(group = 'original')
    ...
    #' Construct tall dataframe + plot individual matrices if requested
    #' use the gene_idx, grna_idx, save_individual_rank_plots in environment
    #' for creating plot_df for individual matrix approximation (+ create individual plot if requested)
    #' @param approxmat (matrix) one (1) matrix approximation e.g. approxmatrices[[r]] or approxmatrices
    #' @param group_name (character) name of the group for this matrix (e.g. method name or rank) the plot will 
    #'     be the saved under this name too: sprintf('%s/%s.pdf', save_folder, group_name)
    inner_format_plotdf <- function(approxmat, group_name) {
      # create df for plotting 
      plot_df = approxmat |> as.data.frame() |> tibble::rownames_to_column('grna') |> 
        tidyr::pivot_longer(cols = 2:(ncol(approxmat)+1), 
                            names_to = 'gene', values_to = 'value') |>
      ...
      if(save_individual_rank_plots) {
        ...
        ggsave(sprintf('%s/%s.pdf', save_folder, group_name), height = 6, width = 3)
      }
    ...
    if(!is.matrix(approxmatrices)) { # if there are ranks
      for(r in ranks) {
        plot_df     = inner_format_plotdf(approxmat=approxmatrices[[r]], group_name=sprintf('rank=%02d', r))
        plot_df_all = rbind(plot_df_all, plot_df)
      }
    } else { # there are no ranks (zeros, average)
        plot_df     = inner_format_plotdf(approxmat=approxmatrices,      group_name=method)
        plot_df_all = rbind(plot_df_all, plot_df)
    }
    ...
    if(!is.null(save_folder) && dir.exists(save_folder)) {
      ...
      plot_df_all$group = factor(
        plot_df_all$group, 
        # levels = c('original', sprintf('rank=%02d', sort(ranks, decreasing=TRUE)))
        levels = c('original', sort(setdiff(unique(plot_df_all$group), 'original'), decreasing=TRUE))
      )
  ...
  # (want to return bc it is useful for shrinkage fn... should prob just make a function for
  #  matrix --> dataframe...)
  return(list(approxmatrices =approxmatrices, # main result
              approxdf=plot_df_all)) # default is NULL, unless created
...
#'        color_limits = c(-2, 2)
#' )
approx_matrix_old <- function(mat, method, ranks, save_plots_filepath=NULL, save_individual_rank_plots=FALSE, color_limits = c(-2, 2), methodParams=NULL) {
