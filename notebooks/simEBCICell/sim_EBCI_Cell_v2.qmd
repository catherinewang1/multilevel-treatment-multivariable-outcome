...
# sim_results$ebci_params$nb$samplesplit$zeros
# sim_results$ebci_params$nb$samplesplit$matcomp_softImpute
...
### pvals from saved sim_result_ebci_pvals.rds-  format to dataframe fishers pvals
```{r}


# load saved ebci pvals under the files <setting name>/<rep number>/sim_result_ebci_pvals.rds
...
# save_folder = sprintf('../../plots/simEBCICell/%s', setting_name)
# sim_results = readRDS(sprintf('%s/1/sim_results.rds', save_folder))
# sim_result_ebci_pvals = readRDS(sprintf('%s/1/sim_result_ebci_pvals.rds', save_folder))
# ranks = sim_results$ranks
...
for(setting_name in c('A', 'E')) {
  ...
  for(cur_save_folder_rep in save_folders_rep[1]) {
    print(sprintf('[%s] %s', Sys.time(), cur_save_folder_rep))
...
h_plot_ebcipval_tall <- function(plot_df, ranks, save_folder) {
  # CI Coverage (should be 1-alpha proportion) # Mis-coverage rate to compare with mse (lower is better)
...
sim_results = readRDS(sprintf('%s/1/sim_results.rds', save_folder))
ranks = sim_results$ranks; rm(sim_results)


# plot_df_one     = readRDS(sprintf('%s/1/sim_result_ebci_pvals_df.rds', save_folder))
plot_df_summary = readRDS(sprintf('%s/sim_result_ebci_summary.rds', save_folder)) # repetition, sim_distn, split_type, method
...
#' @param plot_df_summary (dataframe) that contains the columns: 
...
#' plot_df_summary contains ebci_pvals from 1 run (ebci_pvals) as well as fishers combined ebci pvals (fishers_pval)
#' @param ranks (vector) of integers indicating ranks used
#' @param save_folder (character) path where plots are to be saved at
h_plot_ebcipval <- function(plot_df_summary, ranks, save_folder, height=NULL, width=NULL, save_ggplot=NULL) {
  ...
  p_ebci_pvals = ggplot(temp_df,
                        aes(sample = ebci_pvals, group = methodrank, color = methodrank, fill = methodrank)) +
                geom_abline(aes(slope = 1, intercept = 0)) +
                geom_qq(distribution = stats::qunif, alpha = .5) +
                coord_cartesian(xlim = c(0, 1), ylim = c(0, 1)) +
                labs(title = 'QQ-plot of Inverted EBCI p-values vs Unif(0,1)') + 
...
### plot mse w nice names and labels and colors
```{r}
# setting up nice names and labels
...
# add the corresponding colors if the level is present


length(methodrank_nicenames_order)


# check that these are equal lengths
methodrank_nicenames_order_present = methodrank_nicenames_order[methodrank_nicenames_order %in% unique(plot_df_summ$methodrank)]
length(methodrank_colors) == length(methodrank_nicenames_order_present)
methodrank_colors = 
c( colorRampPalette(c("white", distinct_colors[2]))(2 + 1)[1:2+1], # (original)
  ...
  # colorRampPalette(c("white", distinct_colors[13]))(length(ranks) + 1)[1:length(ranks)+1], # biclustering threshold
  colorRampPalette(c("white", distinct_colors[14]))(2 + 1)[1:2+1] # zeros/avg
  )

names(methodrank_colors) = methodrank_nicenames_order_present
...
distinct(plot_df_summ |> dplyr::ungroup() |> dplyr::select(method, rank, methodrank))
...
# horizantal version (vs vertical)
ggplot() +
...
### plot miscoverage w nice names and labels and colors
```{r}
...
# vertical
ggplot(plot_df_summ, aes(x = methodrank, y = miscoverage_rate, fill = methodrank)) +
...
# horizantal
p = ggplot(plot_df_summ, aes(y = methodrank, x = miscoverage_rate, fill = methodrank)) +
    geom_col(color = 'black') +
...
# save the ggplot objects (to adjust later as needed)
saveRDS(p, sprintf('%s/ggplot_miscoverage_vert.rds', save_folder))
saveRDS(p, sprintf('%s/ggplot_miscoverage_hori.rds', save_folder))
testP = readRDS(sprintf('%s/ggplot_miscoverage.rds', save_folder))
testP
```
...
# load and plot mse for just some subset of sim_results (when ebcipvals weren't calc yet)
source('../../utils/simEBCICell_utils.R') 
...
plot_df = NULL
for(cur_save_folder_rep in save_folder_rep[1:3]) {
  print(sprintf('[%s] %s', Sys.time(), cur_save_folder_rep))
  # create and save ebci pvals for each of the repetitions
  ...
  tryCatch(expr = {
    ...
    plot_df_$filename = cur_save_folder_rep
