...
clustering_names = c('nocluster', 'myKmeans', 'kmeans', 'mclust') # TODO: add myKmeans
...
for(method_name in method_names) {
  ...
  for(clustering_name in clustering_names) {
    ...
    if(clustering_name == 'nocluster') {
          ...
          ebci_results[[method_name]][['nocluster']] = list()
          ebci_results[[method_name]][['nocluster']][['ebci_df']]  = cluster_and_ebci_results$ebci_res
          ebci_results[[method_name]][['nocluster']][['ebci_obj']] = cluster_and_ebci_results$ebci_obj
      ...
      for(k in number_of_clusters) {
        ...
        # if(!(is.na(k) && clustering_name == 'kmeans')) {
          ...
          ebci_results[[method_name]][[paste0(clustering_name, k)]] = list()
          ebci_results[[method_name]][[paste0(clustering_name, k)]][['ebci_df']]  = cluster_and_ebci_results$ebci_res
          ebci_results[[method_name]][[paste0(clustering_name, k)]][['ebci_obj']] = cluster_and_ebci_results$ebci_obj
        # }
...
dir.create('../plots/cellsamplesplit')
dir.create('../plots/cellsamplesplit/clustering/')
dir.create('../plots/cellsamplesplit/shrinkage/')
```
...
for(est_method in names(ebci_results)) {
  for(cl_method in names(ebci_results[[est_method]])) {
    ...
    ggsave(plot     = p_clusters,
           filename = sprintf('../plots/cellsamplesplit/clustering/clustering_%s_%s.pdf', 
...
pdf('../plots/cellsamplesplit/clustering/clustering_all.pdf', height = 10, width = 20)
gridExtra::grid.arrange(grobs = clustering_plots, nrow = 3)
dev.off()

png('../plots/cellsamplesplit/clustering/clustering_all.png', res = 100, units = 'in', height = 10, width = 20)
gridExtra::grid.arrange(grobs = clustering_plots, nrow = 3)
dev.off()
...
for(est_method in names(ebci_results)) {
  ...
  mclust_names = grep(pattern = 'mclust', x = names(ebci_results[[est_method]]), value = TRUE)
...
pdf('../plots/cellsamplesplit/clustering/cluster_probability.pdf', height = 6, width = 12)
...
png('../plots/cellsamplesplit/clustering/cluster_probability.png', res = 100, units = 'in', height = 6, width = 12)
gridExtra::grid.arrange(grobs = clustering_prob_plots,  
                        # layout_matrix = matrix(c(1, 2, 3,
                        #                          4, 5, 6,
                        #                          7, 8, 9), nrow=3)
...
for(est_method in names(ebci_results)) {
  ...
  for(cl_method in names(ebci_results[[est_method]])) {
    ...
    ggsave(plot     = p_shrinkage,
           filename = sprintf('../plots/cellsamplesplit/shrinkage/shrinkage_%s_%s.pdf', est_method, cl_method), 
           height = 5, width = 10)
    }
}


pdf('../plots/cellsamplesplit/shrinkage/shrinkage_all.pdf', height = 10, width = 20)
gridExtra::grid.arrange(grobs = shrinkage_plots, nrow = 3)
dev.off()


png('../plots/cellsamplesplit/shrinkage/shrinkage_all.png', res = 100, units = 'in', height = 10, width = 20)
gridExtra::grid.arrange(grobs = shrinkage_plots, nrow = 3)
dev.off()
```
...
```{r rejection_rates}
# TODO: add a unshrunk results without cellsamplesplitting (larger sample size --> more power?)
...
for(est_method in names(ebci_results)) {
  ...
  for(cl_method in names(ebci_results[[est_method]])) {
    ...
    rejection_rates = rbind(rejection_rates,
                            ebci_results[[est_method]][[cl_method]][['ebci_df']] |> 
                              filter(train01 == 0) |>
...
rejection_rates = rejection_rates |> 
  mutate(shrinkage = factor(shrinkage, 
         ...
         cl_method = factor(cl_method,
                            levels = c('100% cells', '50% cells', names(ebci_results[[1]]))))
...
ggsave('../plots/cellsamplesplit/rejection_rates_tstat_shrinkage.pdf', height = 5, width = 10)
ggsave('../plots/cellsamplesplit/rejection_rates_tstat_shrinkage.png', height = 5, width = 10)
