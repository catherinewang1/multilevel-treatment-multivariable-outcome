...
(put this into an r script `notebooks/runMatrixShrinkage.r`)
...
dir.create('../plots/matrix/shrinkMatrix/heatmaps/')
...
for(filename in list.files("../saves/matrix/shrinkMatrix/")) {
  
  approxmethod = strsplit(x = filename, split = '_')[[1]][[2]]
  rank = gsub(pattern = '\\D', x = filename, replacement='') |> as.numeric()
  
  shrink_dfs[[approxmethod]][[rank]] = read.csv(sprintf("../saves/matrix/shrinkMatrix/%s", filename))
}
...
ggsave(plot = p, filename = '../plots/matrix/shrinkMatrix/heatmaps/shrunkvalue.pdf', height = 7, width = 7)
...
ggsave(plot = p, filename = '../plots/matrix/shrinkMatrix/heatmaps/shrinkagepoint.pdf', height = 7, width = 7)
...
ggsave(plot = p, filename = '../plots/matrix/shrinkMatrix/heatmaps/significant.pdf', height = 7, width = 7)
...
```{r}
# === Shrinkage Changes ===
dir.create('../plots/matrix/shrinkMatrix/points/')
set.seed(12345)
...
ggsave(plot = p, filename = '../plots/matrix/shrinkMatrix/points/shrunkvsunshrunk.pdf', height = 6, width = 7)
...
ggsave(plot = p, filename = '../plots/matrix/shrinkMatrix/points/shrunkvsunshrunkcentered.pdf', height = 6, width = 7)
...
ggsave(plot = p, filename = '../plots/matrix/shrinkMatrix/points/shrinkagepointvsshrunk.pdf', height = 6, width = 7)
...
```{r}
plot_df[subsample_idx, ] |> filter(rank != 'unshrunk') |> mutate(diff = shrunk_value - shrinkage_point) |> pull(diff) |> hist()
```
...
ggsave(plot = p, filename = '../plots/matrix/shrinkMatrix/points/CIs.pdf', height = 6, width = 15)
...
ggsave(plot = p, filename = '../plots/matrix/shrinkMatrix/points/CIlengths.pdf', height = 6, width = 7)
