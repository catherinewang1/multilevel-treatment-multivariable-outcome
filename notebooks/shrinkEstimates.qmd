...
effects_df = read.csv('../saves/sceptre/sceptre_effects_all.csv') |> filter(test == 'discovery') 

topgenes = effects_df |> group_by(gene) |> summarize(sum_effects = sum(abs(fold_change))) |> arrange(desc(sum_effects)) |> head(1000) |> pull(gene)
topgrnas = effects_df |> group_by(grna) |> summarize(sum_effects = sum(abs(fold_change))) |> arrange(desc(sum_effects)) |> head(100 ) |> pull(grna)


saveRDS(topgenes, '../saves/topgenes.rds')
saveRDS(topgrnas, '../saves/topgrnas.rds')
