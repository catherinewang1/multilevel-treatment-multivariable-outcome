...
clustering_names = c('nocluster', 'kmeans', 'mclust')
...
for(method_name in method_names) {
  ...
  effects_train = effects_list[[method_name]][['train']]
  effects_test  = effects_list[[method_name]][['test']]
  ...
  for(clustering_name in clustering_names) {
    ...
    if(clustering_name == 'nocluster') {
          ebci_results[[method_name]][['nocluster']] = cluster_and_ebci_w_split(
      ...
      for(k in number_of_clusters) {
        ...
        # if(!(is.na(k) && clustering_name == 'kmeans')) {
          ebci_results[[method_name]][[paste0(clustering_name, k)]] = cluster_and_ebci_w_split(
                          df_cluster = effects_train, 
                          df_shrink  = effects_test,
                       value_colname = VALUE_COLNAME, 
                          se_colname = SE_COLNAME,
...
dir.create('../plots/clustering_and_shrinkage_cellsamplesplit')
dir.create('../plots/clustering_and_shrinkage_cellsamplesplit/clustering/')
dir.create('../plots/clustering_and_shrinkage_cellsamplesplit/shrinkage/')
```
...
for(est_method in names(ebci_results)) {
  for(cl_method in names(ebci_results[[est_method]])) {
    ...
    shrunk_df = ebci_results[[est_method]][[cl_method]] |>
                filter(train01 == 0)
    ...
    ggsave(plot     = p_clusters,
           filename = sprintf('../plots/clustering_and_shrinkage_cellsamplesplit/clustering/histogram_clustering_%s_%s.pdf', 
...
pdf('../plots/clustering_and_shrinkage_cellsamplesplit/clustering/histogram_clustering_all.pdf', height = 10, width = 20)
...
pdf('../plots/clustering_and_shrinkage/clustering/cluster_probability.pdf', height = 6, width = 12)
...
for(est_method in names(ebci_results)) {
  ...
  for(cl_method in names(ebci_results[[est_method]])) {
    ...
    ggsave(plot     = p_shrinkage,
           filename = sprintf('../plots/clustering_and_shrinkage_cellsamplesplit/shrinkage/shrinkage_%s_%s.pdf', est_method, cl_method), 
           height = 5, width = 10)
    }
}


pdf('../plots/clustering_and_shrinkage_cellsamplesplit/shrinkage/shrinkage_all.pdf', height = 10, width = 20)
...
for(est_method in names(ebci_results)) {
  ...
  for(cl_method in names(ebci_results[[est_method]])) {
    ...
    rejection_rates = rbind(rejection_rates,
                            ebci_results[[est_method]][[cl_method]] |> 
                              filter(train01 == 0) |>
...
ggsave('../plots/clustering_and_shrinkage_cellsamplesplit/rejection_rates_tstat_shrinkage.pdf', height = 5, width = 10)
ggsave('../plots/clustering_and_shrinkage_cellsamplesplit/rejection_rates_tstat_shrinkage.png', height = 5, width = 10)
...
![rejection_rates](./../plots/clustering_and_shrinkage_cellsamplesplit/rejection_rates_tstat_shrinkage.pdf)



![testing inserting image](./../plots/test.pdf)
